...
#[test]
fn shortens_length_of_a_really_short_handshape() {
    let chords = vec![
        Chord { time: 1950, ..Default::default() },
        Chord { chord_id: 1, time: 2000, ..Default::default() },
    ];
    let hs1 = HandShape { chord_id: 0, start_time: 1950, end_time: 2000 };
    let hs2 = HandShape { chord_id: 1, start_time: 2000, end_time: 3000 };
    let mut arr = InstrumentalArrangement {
        ebeats: beats(),
        levels: vec![Level { chords, hand_shapes: vec![hs1, hs2], ..Default::default() }],
...
#[test]
fn lengthens_handshape_when_chord_is_at_end_of_handshape_and_next_anchor_is_very_close() {
    let chords = vec![
        Chord { time: 1000, ..Default::default() },
        Chord { time: 2000, ..Default::default() },
    ];
    let anchors = vec![Anchor { fret: 4, time: 2100, width: 4, end_time: 0 }];
    let hs1 = HandShape { chord_id: 0, start_time: 1000, end_time: 2000 };
    let mut arr = InstrumentalArrangement {
        ebeats: beats(),
        levels: vec![Level { chords, anchors, hand_shapes: vec![hs1], ..Default::default() }],
        ..Default::default()
    };
    lengthen_handshapes(&mut arr);
    assert_eq!(arr.levels[0].hand_shapes[0].end_time, 2050);
}

#[test]
fn test_handshape_handshape_lengthening_with_two_handshapes_lengthen_both() {
    let many_beats: Vec<Ebeat> = (1..=20).map(|i| Ebeat { time: i * 500, measure: -1 }).collect();
    let chords = vec![
        Chord { time: 1000, ..Default::default() },
        Chord { time: 2090, ..Default::default() },
        Chord { time: 3000, ..Default::default() },
        Chord { time: 3500, ..Default::default() },
    ];
    let hs1 = HandShape { chord_id: 0, start_time: 1000, end_time: 2100 };
    let hs2 = HandShape { chord_id: 0, start_time: 3000, end_time: 3500 };
    let mut arr = InstrumentalArrangement {
        ebeats: many_beats,
        levels: vec![Level { chords, hand_shapes: vec![hs1, hs2], ..Default::default() }],
        ..Default::default()
    ...
    lengthen_handshapes(&mut arr);
    assert_eq!(arr.levels[0].hand_shapes[0].end_time, 2225);
    assert_eq!(arr.levels[0].hand_shapes[1].end_time, 3625);
}

#[test]
fn test_handshape_handshape_lengthening_with_two_handshapes_lengthen_second() {
    let many_beats: Vec<Ebeat> = (1..=20).map(|i| Ebeat { time: i * 500, measure: -1 }).collect();
    let chords = vec![
        Chord { time: 1000, ..Default::default() },
        Chord { time: 2000, ..Default::default() },
        Chord { time: 2150, ..Default::default() },
        Chord { time: 2500, ..Default::default() },
    ];
    let hs1 = HandShape { chord_id: 0, start_time: 1000, end_time: 2100 };
    let hs2 = HandShape { chord_id: 0, start_time: 2150, end_time: 2500 };
    let mut arr = InstrumentalArrangement {
        ebeats: many_beats,
        levels: vec![Level { chords, hand_shapes: vec![hs1, hs2], ..Default::default() }],
        ..Default::default()
    ...
    lengthen_handshapes(&mut arr);
    assert_eq!(arr.levels[0].hand_shapes[0].end_time, 2100);
    assert_eq!(arr.levels[0].hand_shapes[1].end_time, 2625);
}
