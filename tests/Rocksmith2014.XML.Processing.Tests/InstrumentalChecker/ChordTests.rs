...
fn detects_chord_note_with_linknext_and_unpitched_slide() {
    ...
    assert!(results
        .iter()
        .any(|r| matches!(r.issue_type(), IssueType::UnpitchedSlideWithLinkNext)));
    assert!(results
        .iter()
        .any(|r| matches!(r.issue_type(), IssueType::LinkNextMissingTargetNote)));
}

#[test]
fn detects_chord_note_with_both_harmonic_and_pinch_harmonic() {
    let cn = vec![ChordNote {
        mask: NoteMask::HARMONIC | NoteMask::PINCH_HARMONIC,
        ..Default::default()
    }];
    let chords = vec![Chord {
        chord_notes: cn,
...
fn detects_harmonic_chord_note_on_7th_fret_with_sustain() {
    ...
    assert!(matches!(
        ...
        IssueType::SeventhFretHarmonicWithSustain
    ));
}
...
fn detects_chord_inside_noguitar_section() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::NoteInsideNoguitarSection
    ));
}
...
fn detects_chord_note_linknext_slide_fret_mismatch() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::LinkNextSlideMismatch
    ));
}
...
fn detects_chord_note_linknext_bend_value_mismatch() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::LinkNextBendMismatch
    ));
}

#[test]
fn detects_missing_bend_value_on_chord_note() {
    let bend_values = vec![BendValue {
        time: 0,
        step: 0.0,
        ..Default::default()
    }];
...
fn detects_linknext_chord_without_any_chord_notes() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::MissingLinkNextChordNotes
    ));
}
...
fn detects_chords_with_weird_fingering() {
    ...
    assert!(results
        .iter()
        .all(|r| matches!(r.issue_type(), IssueType::PossiblyWrongChordFingering)));
}
...
fn detects_chords_with_barre_over_open_strings() {
    ...
    assert!(results
        .iter()
        .all(|r| matches!(r.issue_type(), IssueType::BarreOverOpenStrings)));
}
...
fn detects_non_muted_chords_containing_muted_strings() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::MutedStringInNonMutedChord
    ));
}
...
fn overlapping_bend_values_are_detected_for_chords() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::OverlappingBendValues
    ));
}
...
fn invalid_strings_on_bass_arrangement_are_detected_for_chords() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::InvalidBassArrangementString
    ));
}
...
fn detects_chords_with_techniques_that_require_sustain() {
    let cn1 = vec![ChordNote {
        fret: 1,
        slide_to: 2,
        sustain: 0,
        ..Default::default()
    }];
    let cn2 = vec![ChordNote {
        fret: 1,
        vibrato: 80,
        sustain: 1,
        ..Default::default()
    }];
    let cn3 = vec![ChordNote {
        fret: 1,
        sustain: 2,
        mask: NoteMask::TREMOLO,
        ..Default::default()
    }];
    let cn4 = vec![ChordNote {
        fret: 1,
        slide_unpitch_to: 7,
        sustain: 0,
        ..Default::default()
    ...
    assert!(results
        .iter()
        .all(|r| matches!(r.issue_type(), IssueType::TechniqueNoteWithoutSustain)));
}
...
fn detects_position_shift_into_pull_off_double_stops() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::PositionShiftIntoPullOff
    ));
}
