use rocksmith2014_xml::{
    Anchor, ArrangementEvent as Event, HandShape, InstrumentalArrangement, Level, MetaData, Note,
    Phrase, PhraseIteration, Section, ToneChange,
};
use rocksmith2014_xml_processing::checkers::checker::{
    check_anchors, check_crowd_events, check_handshapes, check_phrases,
};
...
fn detects_unexpected_crowd_speed_event() {
    ...
    assert_eq!(results.len(), 1);
    assert!(
        matches!(results[0].issue_type(), IssueType::EventBetweenIntroApplause(e) if e == "e2")
    );
}
...
fn detects_unexpected_intro_applause_event() {
    ...
    assert_eq!(results.len(), 1);
    assert!(
        matches!(results[0].issue_type(), IssueType::EventBetweenIntroApplause(e) if e == "E3")
    );
}
...
fn detects_unexpected_outro_applause_event() {
    ...
    assert_eq!(results.len(), 1);
    assert!(
        matches!(results[0].issue_type(), IssueType::EventBetweenIntroApplause(e) if e == "D3")
    );
}
...
fn detects_fingering_that_does_not_match_anchor_position() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::FingeringAnchorMismatch
    ));
}
...
fn detects_anchor_inside_handshape() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::AnchorInsideHandShape
    ));
}
...
fn detects_anchor_inside_handshape_at_section_boundary() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::AnchorInsideHandShapeAtPhraseBoundary
    ));
}
...
fn detects_anchor_near_end_of_unpitched_slide() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::AnchorCloseToUnpitchedSlide
    ));
}
...
fn detects_more_than_100_phrases() {
    let mut phrase_iterations = vec![PhraseIteration {
        time: 1000,
        phrase_id: 0,
        ..Default::default()
    }];
    for i in 1..=99 {
        phrase_iterations.push(PhraseIteration {
            time: 1000 + i * 100,
            phrase_id: 1,
            ..Default::default()
        });
