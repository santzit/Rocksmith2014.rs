...
fn detects_unpitched_slide_note_with_linknext() {
    ...
    assert!(matches!(
        ...
        IssueType::UnpitchedSlideWithLinkNext
    ));
}
...
fn detects_linknext_fret_mismatch() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::LinkNextFretMismatch
    ));
}
...
fn fret_numbers_over_24_are_detected() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::FretNumberMoreThan24
    ));
...
fn natural_harmonic_with_bend_is_detected() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::NaturalHarmonicWithBend
    ));
}
...
fn invalid_strings_on_bass_arrangement_are_detected() {
    ...
    assert!(results
        .iter()
        .all(|r| matches!(r.issue_type(), IssueType::InvalidBassArrangementString)));
}
...
fn detects_phrase_on_linknext_notes_sustain() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::PhraseChangeOnLinkNextNote
    ));
}
...
fn detects_finger_change_during_slide() {
    ...
    assert!(matches!(
        results[0].issue_type(),
        IssueType::FingerChangeDuringSlide
    ));
}
