...
impl SngRead for Chord {
    fn sng_read<R: Read>(r: &mut R) -> crate::Result<Self> {
        ...
        let mut frets = [0i8; 6];
        for v in frets.iter_mut() { *v = read_i8(r)?; }
        let mut fingers = [0i8; 6];
        for v in fingers.iter_mut() { *v = read_i8(r)?; }
        let mut notes = [0i32; 6];
        for v in notes.iter_mut() { *v = read_i32(r)?; }
        let mut name = [0u8; 32];
...
impl SngWrite for Chord {
    fn sng_write<W: Write>(&self, w: &mut W) -> crate::Result<()> {
        ...
        for &v in &self.frets { write_i8(w, v)?; }
        for &v in &self.fingers { write_i8(w, v)?; }
        for &v in &self.notes { write_i32(w, v)?; }
